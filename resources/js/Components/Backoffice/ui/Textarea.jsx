export default function Textarea({ label, hint, error, required = false, className = '', ...props }) {
    return (
        <label className={`block space-y-2 ${className}`}>
            ...
            <textarea
                className={`min-h-32 w-full rounded-2xl border bg-slate-900/80 px-4 py-3 text-sm text-white outline-none transition placeholder:text-white/25 disabled:cursor-not-allowed disabled:opacity-60 ${
