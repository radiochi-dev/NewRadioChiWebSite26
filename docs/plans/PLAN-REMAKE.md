...
- ~~Ajuste posterior del rollout multidioma cerrado para `settings` y `music-tracks`: el editor principal ya cambia el contenido traducible por locale, persiste `settings_translations` / `music_track_translations` desde el mismo formulario y mantiene los assets de imagen como fuente compartida entre idiomas.~~
- ~~Refinamiento posterior del editor page-centric cerrado para `pages/home`: desaparece el bloque `Modo EDIT`, los slides pasan a tabs inline dentro del mismo contenedor y la propia pagina ya puede crear/eliminar bloques editoriales sin volver a una lista secundaria.~~
- ~~Limpieza visual global aplicada despues en formularios del backoffice: se retiran el panel y la tarjeta-resumen `Modo EDIT` de la shell CRUD reutilizable para liberar altura y eliminar ruido redundante.~~
- ~~Ajuste visual posterior aplicado sobre campos compartidos del backoffice: iconos nativos de fecha/hora, checks de tablas/toggles y SVG de controles quedan reequilibrados para asegurar contraste correcto en tema oscuro.~~
- ~~Refinamiento visual posterior de interacciones del backoffice: todos los botones abandonan gradients y quedan unificados con un solo color de acento, reposo transparente y hover/focus/active relleno para marcar mejor el cambio de estado.~~
- ~~Refinamiento posterior del selector de idiomas del backoffice: el locale activo queda marcado de forma persistente en las barras de idiomas y `ES` actua como fallback visual por defecto cuando no venga uno seleccionado en el payload.~~
